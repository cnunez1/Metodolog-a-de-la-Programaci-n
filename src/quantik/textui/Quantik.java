...
public class Quantik {
	 ...
	 * @throws IllegalArgumentException si el argumento con el modo de deshacer no es correcto
	 */
	private static String extraerModoDeshacer(String[] args) {
		if (args.length >= 1) {
			...
			if (!modo.equals("jugadas") && !modo.equals("partidas")) {
				throw new IllegalArgumentException("Error en argumento de entrada, modo " 
										+ modo + " no disponible");
			}
			return modo;
		}
